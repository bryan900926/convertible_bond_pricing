                                      ...
                                      const CouponPaidInfo &coupon_info)
{
    ...
    Eigen::Array<int, Eigen::Dynamic, 9> nxt_p(l_data.size(), 9);
    ...
    for (int h = 0; h < l_data.size(); ++h)
    {
        ...
        const Eigen::ArrayXd l_vec = l_data[h].l_min * ratio_min_vec + l_data[h].l_max * ratio_max_vec;

        const double r_t = tree_result.short_rate_tree(static_cast<int>(l_data[h].k), static_cast<int>(l_data[h].step) - 1);
    ...
    Eigen::ArrayXXd equity_tree = EquityFunVec(
                                      cb_paras,
                                      ctx,
                                      v_data) /
                                  cb_paras.NS;
    ...
    // first step
    {
        ...
        jump = jump_first;
        const Eigen::ArrayXd l_vec = l_data[0].l_min * ratio_min_vec + l_data[0].l_max * ratio_max_vec;

        const double r_t = tree_result.short_rate_tree(static_cast<int>(l_data[0].k), static_cast<int>(l_data[0].step) - 1);
        ...
        v_data_first(0) = v_t;
        equity_tree(0, Eigen::all) = EquityFunVec(
                                         cb_paras,
                                         EquityContextVec(
                                             dt,
                                             l_data_first,
    ...
    return {std::move(equity_tree),
            std::move(idx_vec),
            std::move(nxt_m),
            std::move(nxt_p),
            std::move(l_data_partition)};
}
