                             ...
                             const Eigen::ArrayXd &v_t_arr)
{
  ...
  if (std::abs(ctx.corr_xy) > 1e-5)
  {
    ...
    for (int i = 0; i < gh_result.x.size(); ++i)
    {
      const double z = gh_result.x(i) * SQRT_2;
      const double w = gh_result.w(i);

      d1_arr = d1_static + (d1_sign_h * z);
      d2_arr = d2_static + (d2_sign_h * z); 

      d1_arr = (1.0 + (-1.702 * d1_arr).exp()).inverse(); // approximation to N(d1)
      d2_arr = (1.0 + (-1.702 * d2_arr).exp()).inverse();
      
      double scalar_exp_z = std::exp(c_yy_sqrt * z); // Scalar exponent

      integral_arr += w * (
                            (d1_arr * exp_part1_base) -
