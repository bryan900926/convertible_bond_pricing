...
void CbTreePricing(const CbParas &cb_paras, const CdgParas &cdg_paras, const VasciekParas vasciek_paras)
{
    ...
    PzTreeResult pz_result = PzTreeBuild(
        tree_result.short_rate_tree,
        coupon_info.total_steps,
        tree_result.j_max_other);

    const Eigen::ArrayXd &thetas = tree_result.alpha_result.thetas;
    const std::vector<Eigen::ArrayX3i> &nxt_r_idx_mat = pz_result.nxt_idx_mat;
    ...
    for (int i = 1; i <= coupon_info.total_steps; ++i)
    {
        ...
        for (int count = level_start_idx; count <= level_end_idx; ++count)
        {
            ...
            const double l_hat = (cdg_paras.delta + cdg_paras.sigma_v * cdg_paras.sigma_v / 2) / cdg_paras.lamda -
                                 cdg_paras.v + cdg_paras.phi * vasciek_paras.r_bar - r_now * (1 / cdg_paras.lamda + cdg_paras.phi);
            ...
            const double miu_y = r_now - cdg_paras.delta - cdg_paras.sigma_v * cdg_paras.sigma_v / 2;
            ...
            for (int j = 0; j < 9; ++j)
            {
                ...
                const double k_next = nxt_r_idx_mat[i - 1](k_now, (j / 3));
                const long long key = (static_cast<long long>(m_next) << 32) | (static_cast<long long>(k_next));
