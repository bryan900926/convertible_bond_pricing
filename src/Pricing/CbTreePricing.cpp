...
void CbTreePricing(const CbParas &cb_paras, const CdgParas &cdg_paras, const VasciekParas vasciek_paras)
{
    ...
    l_data.emplace_back(
        LNode{
            1, static_cast<size_t>(pz_result.start_h), 0, cdg_paras.l0, cdg_paras.l0});
    ...
    // m in [-2n, 2n]
    std::vector<std::vector<int>> l_idx_tracker((n + 1) * 4 + 2, std::vector<int>(tree_result.short_rate_tree.rows(), -1));
    ...
    for (size_t i = 1; i <= n; ++i)
    {
        ...
        for (int count = level_start_idx; count <= level_end_idx; ++count)
        {
            ...
            PNode p_node = {i, k_now, m_now, {{p_u * p_u_hw, p_u * p_m_hw, p_u * p_d_hw}, {p_m * p_u_hw, p_m * p_m_hw, p_m * p_d_hw}, {p_d * p_u_hw, p_d * p_m_hw, p_d * p_d_hw}}};
            ...
            for (int j = 0; j < 9; ++j)
            {
                ...
                const int m_next_query = m_next + l_idx_offset;
                const int key = l_idx_tracker[m_next_query][k_next];
                    ...
                    if (key == -1)
                    {
                        l_data.emplace_back(
                            LNode{
                                ...
                                NaN,
                                NaN});
                        l_idx_tracker[m_next_query][k_next] = ++idx_total;
                    }
                    continue;
                }

                const double x_nxt = x0 + m_next * jump;
                const double y_nxt = y0 + (x_nxt - x0) + cdg_paras.sigma_v * cb_paras.rho * (tree_result.short_rate_tree(k_next, i) - vasciek_paras.r0) / vasciek_paras.sigma_r;
                ...
                {
                    LNode &exist_node = l_data[l_idx_tracker[m_next_query][k_next]];
