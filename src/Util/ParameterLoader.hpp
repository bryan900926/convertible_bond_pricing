...
public:
    ...
    static ParameterLoader StartLoadParams(const std::string& ticker,
      ...
      rapidcsv::Document doc(parameter_csv_path, rapidcsv::LabelParams(0, 0),
        ...
        // --- Extract CbParas ---
        cb.T = doc.GetCell<double>(ParameterColumn::T, i); // Maturity of the
                                                           // bond in years
        PARAMETER_CHECK(cb.T > 0,
                        "Maturity (T) must be positive for ticker: " + ticker);
        cb.F           = doc.GetCell<double>(ParameterColumn::F, i);            // Face value of the bond
        PARAMETER_CHECK(cb.F > 0,
                        "Face value (F) must be positive for ticker: " + ticker);   
        ...
        PARAMETER_CHECK(cb.NS > 0,
                        "Number of shares (NS) must be positive for ticker: " +
                            ticker);
        ...
        cb.dt_other = doc.GetCell<double>(
            ParameterColumn::dt_other, i); // Time step for the tree, in years

        PARAMETER_CHECK(cb.dt_other > 0,
                        "Time step (dt_other) must be positive for ticker: " +
                            ticker);
        ...
        cdg.l0 = doc.GetCell<double>(
            ParameterColumn::l0,
            i); // Initial leverage ratio (should be negative)

        PARAMETER_CHECK(cdg.l0 < 0,
                        "l0 must be negative for ticker: " + ticker);

        cdg.V0 = doc.GetCell<double>(ParameterColumn::V0, i);

        PARAMETER_CHECK(cdg.V0 > 0,
                        "V0 must be positive for ticker: " + ticker);
        
        cdg.delta   = doc.GetCell<double>(ParameterColumn::delta, i);
        cdg.sigma_v = doc.GetCell<double>(ParameterColumn::sigma_v, i);

        PARAMETER_CHECK(cdg.sigma_v > 0,
                        "sigma_v must be positive for ticker: " + ticker);
        ...
        PARAMETER_CHECK(vas.sigma_r > 0,
                        "sigma_r must be positive for ticker: " + ticker);
        
        vas.r0      = doc.GetCell<double>(ParameterColumn::r0, i);

        // 2. Removed LabelParams(0, 0) here because multiple rows share the same ticker!
        rapidcsv::Document call_doc(schedule_path);
        
        std::vector<ScheduleInfo> call_infos;
        ...
        for (size_t r = 0; r < row_count; ++r) {
            ...
            double price =
                call_doc.GetCell<double>(ParameterColumn::CallPrice, r);
            std::string type = call_doc.GetCell<std::string>(ParameterColumn::Type, r);
            PARAMETER_CHECK(type == "Call" || type == "Put", "Invalid option type for ticker: " + ticker);
            call_infos.emplace_back(date, price, type == "Call" ? ScheduleInfo::ScheduleType::Call : ScheduleInfo::ScheduleType::Put);
        }
    ...
    void Pricing(const std::string& path, FileMode mode = FileMode::Overwrite) {
