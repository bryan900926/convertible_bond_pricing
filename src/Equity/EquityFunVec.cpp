    ...
    const Eigen::ArrayXd &v_t_arr)
{
    ...
    if (std::abs(ctx.corr_xy) > 1e-10)
    {
        ...
        const double c_yy_cond_sqrt = std::sqrt(ctx.c_yy * (1.0 - ctx.corr_xy * ctx.corr_xy));
        ...
        for (size_t k = 0; k < cb_paras.qdt; ++k)
        {
            ...
            const Eigen::ArrayXXd part2 = Eigen::exp(
                                              ctx.n_arr +
                                              ctx.m_arr +
                                              c_yy_cond_sqrt +
                                              part_2_fourth) *
    ...
    else
    {
        ...
        Eigen::ArrayXXd first_arr = (1.0 - term_B.exp()) * (term_A.exp().colwise() * (v_t_arr * ctx.pt_arr));

        Eigen::ArrayXXd x_arr = -ctx.n_arr / std::sqrt(ctx.c_yy) - std::sqrt(ctx.c_yy);
