...
double CalculateEquityNode(double v_t, double l_t, double r_t, double dt,
  ...
  const double bond_term_1 =
      (r_bar - sigma_r_sq / (2.0 * K * K)) * (Bk_dt - dt);
  ...
  const double term_ht_3 =
      (ld * phi / K) * ((theta_t1 + exp_ld_dt * theta_t) / 2.0) * dt;
  const double term_ht_4 =
      ld * b *
      (exp_ld_dt * Bk_ld_dt * r_t + exp_K_dt * Bk_ld_dt * theta_t * dt / 2.0);

  const double ht = term_ht_1 + term_ht_2 + term_ht_3 + term_ht_4;

  const double M = Mt - delta * dt - sigma_v_sq / 2.0 * dt -
                   (sigma_r_sq / (K * K)) * dt_combo -
                   sigma_v * sigma_r * rho / K * (dt - Bk_dt);

  const double term_N_1 = (ld * b * sigma_r_sq) / (K * (ld - K)) *
                          (Bk_dt - B_ld_dt - Bk_2dt + B_ld_plusK_dt);
  const double term_N_2 =
      sigma_v * sigma_r * rho / K * (B_ld_dt - B_ld_plusK_dt);

  const double N = ht - term_N_1 + term_N_2;

  const double Cxx = (sigma_r_sq / (K * K)) * dt_combo +
                     2.0 * sigma_v * sigma_r * rho / K * (dt - Bk_dt) +
                     sigma_v_sq * dt;
  if (Cxx <= 0) {
  ...
  const double term_Cyy_1 = term_Cyy_coeff * term_Cyy_coeff *
                            (Bk_2dt - 2.0 * B_ld_plusK_dt + B_2ld_dt);
  const double term_Cyy_2 = 2.0 * ld * b * sigma_v * sigma_r * rho / (ld - K) *
                            (B_ld_plusK_dt - B_2ld_dt);
  ...
  // --- 10. Cxy ---
  const double term_Cxy_1 = ld * b * sigma_r_sq / ((ld - K) * K) *
                            (Bk_dt - B_ld_dt - Bk_2dt + B_ld_plusK_dt);
  const double term_Cxy_2 =
      sigma_v * sigma_r * rho / K * (B_ld_dt - B_ld_plusK_dt);
  const double term_Cxy_3 =
      ld * b * sigma_v * sigma_r * rho / (ld - K) * (Bk_dt - Bk_ld_dt);
  ...
  // --- 12. Pt ---
  const double Bk_sq_integral =
      (2.0 * K * dt - std::exp(-2.0 * K * dt) + 4.0 * std::exp(-K * dt) - 3.0) /
      (2.0 * K * K * K);
  ...
  if (std::abs(corr_xy) > 1e-9) // Non-zero correlation
  {
    ...
    double integral_sum =
        FormulaFastScalar(M, N, Cxx, Cyy, corr_xy, gh_rule.x, gh_rule.w);
  ...
  } else // Zero Correlation (Closed Form)
  {
    ...
    equity_val = Pt * v_t * std::exp(M + Cxx / 2.0) *
                 (1.0 - std::exp(N + Cyy / 2.0)) * normcdf_z;
  }
