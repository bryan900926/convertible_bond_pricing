                                      ...
                                      EquityManager &equity_manager)
{
    ...
    for (int h = 0; h < num_nodes; ++h)
    {
        ...
        equity_manager.t_vec(h) = t * cb_paras.dt_other + coupon_info.dt_first;
        equity_manager.alpha_vec(h) = tree_result.alpha_result.alphas(t);
        ...
        double v_t = std::exp(exponent);
        equity_manager.l_t_arr.row(h).segment(0, cb_paras.partition) = l_vec.transpose();
        equity_manager.r_t_arr(h) = r_t;
        equity_manager.theta_t_arr(h) = thetas(t);
        equity_manager.theta_t1_arr(h) = thetas(t + 1);
        equity_manager.v_t_arr(h) = v_t;
    }
    equity_manager.update_context();
}